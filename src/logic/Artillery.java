...
public class Artillery extends Unit {
	...
	public void draw(GraphicsContext gc) {
		...
		if (direction == 1)
			gc.drawImage(RenderableHolder.artillery[0], this.x, this.y);
		if (direction == -1)
			gc.drawImage(RenderableHolder.artillery[1], this.x, this.y);

	}

	public void hit(Unit u) {
		if (u instanceof Soldier)
			this.health = this.health - ((Soldier) u).getFirepower() * 3;
		else if (u instanceof Tank)
			this.health = this.health - ((Tank) u).getFirepower();
		else if (u instanceof APC)
			this.health = this.health - ((APC) u).getFirepower();
